...
В рамках этапа 2 моей задачей было подобрать техническое решение для реализации выбранной гипотезы, оценить возможность технической реализации идеи, адаптировать существующий прототип под требования проекта и подготовить материалы для презентации.
...
- системный анализ и финальные требования: [system_analysis.md](/home/texnik71rus/projects/trainer_for_kids/system_analysis.md);
- исходный код приложения: [spelling_trainer](/home/texnik71rus/projects/trainer_for_kids/spelling_trainer);
- контент и изображения: использованы готовые слова, аудио и изображения из текущего приложения.
...
- в проекте уже существовала рабочая кодовая база, которую можно было адаптировать под новую гипотезу;
- решение позволяет быстро собрать интерактивный прототип без разработки с нуля;
...
- в текущем проекте уже нет готовой web-основы.
...
- потребовался бы перенос существующей логики в новый стек;
...
Это решение оказалось наиболее подходящим, потому что позволило использовать уже существующий рабочий код, быстро адаптировать приложение под финальную гипотезу и получить не просто набор экранов, а интерактивный прототип с реальной логикой пользовательского сценария.
...
- за основу был взят существующий прототип орфографического тренажера;
- приложение было адаптировано под гипотезу про реальные награды;
- в интерфейс был добавлен блок `Моя цель`;
- был добавлен прогресс-бар достижения цели;
- был зафиксирован режим награды в баллах;
- была добавлена возможность изменить цель;
- была добавлена возможность подтвердить получение награды родителем;
- был сохранен основной сценарий тренировки слов, статистики и настроек.

Таким образом, существующий код был не переписан с нуля, а доработан под финальную концепцию продукта.
...
На этапе 2 по роли разработчика я занимался подбором технического решения и адаптацией прототипа под выбранную гипотезу. Для реализации был выбран стек `Python + PySide6`, потому что в проекте уже существовал рабочий код орфографического тренажера, который можно было быстро доработать под новую концепцию. В прототипе был реализован основной пользовательский сценарий: ребенок проходит тренировку, получает баллы, видит прогресс к цели и после достижения цели родитель подтверждает награду. Это позволяет сделать вывод, что идея проекта технически реализуема и может быть развита в полноценный продукт.
